...
export const generatePdf = (datas: RelatorioData[]) => {
    const { jsPDF } = jspdf;
    const doc = new jsPDF();
    ...
    datas.forEach((data, index) => {
        ...
        // Seção 1.2: Análise de Solo
        let lastY = (doc as any).lastAutoTable.finalY;
        if (data.analise_solo && Object.values(data.analise_solo).some(v => v != null)) {
            doc.setFontSize(14);
            doc.text('1.2 Análise de Solo', 14, lastY + 15);
            
            const {
                ph_solo, fosforo_ppm, potassio_ppm, calcio_cmolc,
                magnesio_cmolc, aluminio_cmolc, materia_organica_percent
            } = data.analise_solo;

            // Primeira linha de dados da análise
            doc.autoTable({
                startY: lastY + 20,
                head: [['pH do Solo', 'Fósforo (ppm)', 'Potássio (ppm)', 'Cálcio (cmolc/dm³)']],
                body: [[
                    ph_solo ?? 'N/A',
                    fosforo_ppm ?? 'N/A',
                    potassio_ppm ?? 'N/A',
            ...
            // Segunda linha de dados da análise
            doc.autoTable({
                startY: (doc as any).lastAutoTable.finalY,
                head: [['Magnésio (cmolc/dm³)', 'Alumínio (cmolc/dm³)', 'Matéria Orgânica (%)']],
                body: [[
                    magnesio_cmolc ?? 'N/A',
                    aluminio_cmolc ?? 'N/A',
                    materia_organica_percent ?? 'N/A'
                ]],
                theme: 'grid',
                headStyles: { fillColor: [100, 100, 100] },
            });
        ...
        doc.setFontSize(16);
        doc.text('2. Resumo de Indicadores', 14, lastY + 15);
        doc.autoTable({
            startY: lastY + 20,
