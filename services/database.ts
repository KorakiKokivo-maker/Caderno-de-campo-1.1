// FIX: Changed import to relative path and added all necessary types.
...
// Mock Data
// FIX: Added 'foto_url' to user objects to match the Usuario type and application usage.
let users: Usuario[] = [
  { id: 1, nome: 'João da Silva', nome_de_usuario: 'joao', senha: '123', perfil: 'Produtor', foto_url: '' },
  { id: 2, nome: 'Ana Souza', nome_de_usuario: 'ana', senha: '123', perfil: 'Técnico', foto_url: '' },
];
...
// FIX: Added updateUser function to resolve missing export error in ConfiguracoesScreen.
export const updateUser = (userId: number, updatedData: Partial<Usuario>): Usuario | undefined => {
    const index = users.findIndex(u => u.id === userId);
    if (index !== -1) {
        // Prevent password from being overwritten if not provided
        const { senha, ...rest } = updatedData;
        users[index] = { ...users[index], ...rest };
        
        const { senha: currentPassword, ...userWithoutPassword } = users[index];
        return userWithoutPassword;
    }
    return undefined;
};
...
export const updateSafra = (id: number, updatedData: Partial<Omit<Safra, 'id'>>): Safra | undefined => {
    const index = safras.findIndex(s => s.id === id);
    if (index !== -1) {
        safras[index] = { ...safras[index], ...updatedData };
        return safras[index];
    }
    return undefined;
};
...
export const reactivateSafra = (id: number): void => {
    const index = safras.findIndex(s => s.id === id);
    if (index !== -1) safras[index].ativa = true;
};
...
export const addPropriedade = (propriedade: Omit<Propriedade, 'id'>): Propriedade => {
    const newProp = { ...propriedade, id: propriedades.length + 1 };
