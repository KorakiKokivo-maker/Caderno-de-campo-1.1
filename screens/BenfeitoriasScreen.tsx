...
const BenfeitoriaForm: React.FC<{
    ...
    const [formData, setFormData] = useState<Omit<Benfeitoria, 'id'>>({
        ...
        data_inicio_pagamento: editData?.data_inicio_pagamento || new Date().toISOString().split('T')[0],
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if(editData) {
            updateBenfeitoria(editData.id, formData);
        } else {
    ...
    return (
         <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg m-4 max-h-[90vh] overflow-y-auto">
                <h2 className="text-2xl font-bold mb-4">{editData ? 'Editar Benfeitoria' : 'Adicionar Benfeitoria'}</h2>
...
const BenfeitoriasScreen: React.FC<{
  ...
  showToast: (message: string, type?: 'success' | 'error') => void;
    ...
    const [editItem, setEditItem] = useState<Benfeitoria | null>(null);

    const refreshBenfeitorias = () => setBenfeitorias(getBenfeitoriasByUsuario(currentUser));
    useEffect(refreshBenfeitorias, [currentUser]);

    const handleSave = () => {
        showToast(editItem ? "Benfeitoria atualizada com sucesso!" : "Benfeitoria adicionada com sucesso!", "success");
        setIsFormOpen(false);
        setEditItem(null);
        refreshBenfeitorias();
    }
    ...
    const handleDelete = (item: Benfeitoria) => {
        if(confirm(`Deseja realmente excluir a benfeitoria "${item.nome}"?`)) {
            deleteBenfeitoria(item.id);
            showToast("Benfeitoria excluída com sucesso!", "success");
            refreshBenfeitorias();
        }
    }
    ...
    return (
        <ScreenWrapper title="Benfeitorias e Estruturas" onNavigateBack={onNavigateBack}>
            {isFormOpen && <BenfeitoriaForm currentUser={currentUser} onClose={() => { setIsFormOpen(false); setEditItem(null); }} onSave={handleSave} editData={editItem || undefined} />}
                            ...
                            {benfeitorias.length > 0 ? benfeitorias.map(b => 
                                <tr key={b.id} className="border-b dark:border-gray-700">
                                    ...
                                    <td className="text-center flex justify-center gap-2">
                                        <button onClick={() => handleEdit(b)} className="px-2 py-1 bg-yellow-500 text-white rounded-md hover:bg-yellow-600">
                                            <Icon name="pencil" className="w-4 h-4"/>
                                        </button>
                                        <button onClick={() => handleDelete(b)} className="px-2 py-1 bg-red-500 text-white rounded-md hover:bg-red-600">
                                            <Icon name="trash" className="w-4 h-4"/>
                                        </button>
                                    </td>
                                </tr>
                            ) : (
                                <tr><td colSpan={5} className="text-center py-4">Nenhuma benfeitoria registrada.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
